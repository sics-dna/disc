...
public:
	IscMultiGaussianMicroModel(int d, int* iv) {
		int i;
		dim = d;
		indv = new int[d];
		sumx = new double[d];
		sumxx = new HMatrix(d,2);
		mean = new double[d];
		var = new HMatrix(d,2);
		dirty = 1;
		n = 0.0;
		prior_n = 0.0;
		prior_var = 0.0;
		for (i=0; i<d; i++) indv[i] = iv[i], sumx[i] = 0.0;
		if(DEBUG)
			printf("IscMultiGaussianMicroModel created\n");
	};
	virtual ~IscMultiGaussianMicroModel() {
		delete [] indv;
		delete [] sumx;
		delete sumxx;
		delete [] mean;
		delete var;
		if(DEBUG)
			printf("IscMultiGaussianMicroModel deleted\n");
	};
